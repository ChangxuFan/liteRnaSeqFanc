# modeled after ~/R_packages/wgbsFanc/R/simulate_reads.R
simulate.reads <- function(regions.gr, name.col = NULL, genome, 
                                template.sizes, read.length, convert.rate, seed = NULL,
                                convert.strand = c("T", "B"), out.root, suffix = ".fastq",
                           threads = 1,
                           bedtools = BEDTOOLS) {
  ...
  if (any(width(regions.gr) < max(template.sizes))) {
    stop("any(width(regions.gr) < max(template.sizes))")
  }
  utilsFanc::t.stat("getting sequences")
  
  seqs.by.region <- utilsFanc::safelapply(1:length(regions.gr), function(i) {
    region <- regions.gr[i]
    print(paste0("getting sequences for region: ", region))
    mate1.strand <- "+"
    mate2.strand <- "-"
    df <- lapply(template.sizes, function(template.size) {
    ...
    seqs <- utilsFanc::safelapply(c(1, 2), function(i) {
      ...
      seq.vec <- abaFanc2::get.fasta.gr(gr = gr.mate, genome = genome,
                                        as.string = T, id.col = "regionID", memory.mode = F, 
                                        threads = threads,
                                        print.cmd = F, bedtools = bedtools) %>% unlist()
      return(seq.vec)
    }, threads = 2)
    names(seqs) <- c("R1", "R2")
    return(seqs)
  ...
  if (convert.rate == 0) {
    fastqs <- liteRnaSeqFanc::fastq.write(seq.list = seqs.by.strand, out.root = paste0(out.root, "_R"), 
                                          out.suffix = suffix, zip = T)
    filenames <- paste0(out.root, "_", names(seqs.by.strand), suffix, ".gz")
    return(filenames)
  } else{
